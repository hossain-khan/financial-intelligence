...
- Browser-local provider selected after capability/license/quality benchmarks. (Landed in #33: `@financial-intelligence/ai-local` using transformers.js in a worker, sideload-only acquisition with SHA-256 integrity, capability preflight, and offline execution. The model profile is now pinned to Gemma 3n E2B (ONNX, dtype q4) after a WebGPU spike; a full #32 corpus evaluation of the pinned model is still pending. See ADR-020.)
