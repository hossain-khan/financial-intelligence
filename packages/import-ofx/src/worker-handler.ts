...
export type OfxImportWorkerHandler = (message: unknown) => Promise<void>;

export function createOfxImportWorkerHandler(
  target: WorkerResponseTarget,
  parser: StatementParser = new OfxStatementParser(),
): OfxImportWorkerHandler {
...
function normalizeFailure(
  ...
  if (aborted || (error instanceof OfxImportError && error.code === "CANCELLED")) {
    return failed(operationId, "CANCELLED", "Statement parsing was cancelled");
  }
  if (error instanceof OfxImportError) {
